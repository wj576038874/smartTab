...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    private float tabViewSelectedTextSize;
    ...
    private ArgbEvaluator evaluator; // ARGB求值器
    ...
    public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        selectedTextSize = a.getDimension(
                R.styleable.stl_SmartTabLayout_stl_selectedTabTextSize, textSize);
        ...
        a.recycle();
        evaluator = new ArgbEvaluator(); // ARGB求值器
        ...
        this.tabViewSelectedTextSize = selectedTextSize;
        mMinScale = tabViewTextSize / tabViewSelectedTextSize;
    ...
    public void setTabSelectedTextSize(int textSize) {
        int tabTextSize = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, textSize, getResources().getDisplayMetrics());
        this.tabViewSelectedTextSize = textSize;
        //计算出比例
        mMinScale = tabViewTextSize / tabTextSize;
    }
     ...
     */
    protected View createDefaultTabView(ViewGroup container, int position, PagerAdapter adapter) {

        setCustomTabView(R.layout.smart_tab_default_layout_item, R.id.tv_smart_tab);

        return tabProvider.createTabView(container, position, adapter);
...
//        return textView;
    ...
    private void populateTabStrip() {
        ...
        for (int i = 0; i < adapter.getCount(); i++) {
            ...
            final View tabView = (tabProvider == null)
                    ? createDefaultTabView(tabStrip, i, adapter)
            ...
            if (i == viewPager.getCurrentItem()) {
                ...
                TextView textView = tabView.findViewById(R.id.tv_smart_tab);
                textView.setScaleX(1 + (1 - mMinScale));
                textView.setScaleY(1 + (1 - mMinScale));
            }
        }
    }
    ...
    private class SimpleTabProvider implements TabProvider {
        ...
        public View createTabView(ViewGroup container, int position, PagerAdapter adapter) {
            ...
            if (tabTitleView != null) {
                ...
                tabTitleView.setTextSize(TypedValue.COMPLEX_UNIT_PX, tabViewTextSize);
                tabTitleView.setTypeface(Typeface.DEFAULT_BOLD);
                tabView.setLayoutParams(new LinearLayout.LayoutParams(
                        LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.MATCH_PARENT));

                if (tabViewBackgroundResId != NO_ID) {
                    tabView.setBackgroundResource(tabViewBackgroundResId);
                } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                    // If we're running on Honeycomb or newer, then we can use the Theme's
                    // selectableItemBackground to ensure that the View has a pressed state
                    TypedValue outValue = new TypedValue();
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                    // If we're running on ICS or newer, enable all-caps to match the Action Bar tab style
                    tabTitleView.setAllCaps(tabViewTextAllCaps);
                }
                ...
                if (tabViewTextMinWidth > 0) {
                    tabTitleView.setMinWidth(tabViewTextMinWidth);
                }
            }
    ...
    private class InternalViewPagerListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
            ...
            if (positionOffset > 0) {
                View currentTab = tabStrip.getChildAt(position).findViewById(R.id.tv_smart_tab);
                View sideTab = tabStrip.getChildAt(position + 1).findViewById(R.id.tv_smart_tab);

                if (currentTab instanceof TextView && sideTab instanceof TextView) {
                    //颜色渐变
                    int defaultColor = tabViewTextColors.getDefaultColor();
                    int stateColor = tabViewTextColors.getColorForState(getDrawableState(), defaultColor);

                    //选中渐变未选中
                    int evaluateCurrent = (Integer) evaluator.evaluate(positionOffset, defaultColor, stateColor);
                    ((TextView) currentTab).setTextColor(evaluateCurrent);

                    //未选中渐变选中
                    int evaluateSide = (Integer) evaluator.evaluate(positionOffset, stateColor, defaultColor);
                    ((TextView) sideTab).setTextColor(evaluateSide);


                    //字体大小渐变
                    if (mMinScale >= 1.0f) return;
//                    float textSizeLeft = tabSelectTextSize - (tabSelectTextSize - tabViewTextSize) * positionOffset;
//                    ((TextView) currentTab).setTextSize(TypedValue.COMPLEX_UNIT_PX, Math.round(textSizeLeft));

//                    currentTab.setScaleX(mMinScale + (1.0f - mMinScale) * (1 - positionOffset));
//                    currentTab.setScaleY(mMinScale + (1.0f - mMinScale) * (1 - positionOffset));

                    currentTab.setScaleX(1 + (1.0f - mMinScale) * (1 - positionOffset));
                    currentTab.setScaleY(1 + (1.0f - mMinScale) * (1 - positionOffset));

//                    float textSizeRight = (tabSelectTextSize - tabViewTextSize) * positionOffset + tabViewTextSize;
//                    ((TextView) sideTab).setTextSize(TypedValue.COMPLEX_UNIT_PX, Math.round(textSizeRight));
//                    QLog.e("www" , Math.round(textSizeLeft)+"***"+Math.round(textSizeRight)+"***"+positionOffset);

//                    sideTab.setScaleX(mMinScale + (1.0f - mMinScale) * positionOffset);
//                    sideTab.setScaleY(mMinScale + (1.0f - mMinScale) * positionOffset);

                    sideTab.setScaleX(1 + (1.0f - mMinScale) * positionOffset);
                    sideTab.setScaleY(1 + (1.0f - mMinScale) * positionOffset);
...
//                        if (position == i) {
//                            ((TextView) tabView).setTextSize(TypedValue.COMPLEX_UNIT_PX, tabViewTextSize);
//                        } else {
//                            tabView.setScaleX(mMinScale);
//                            tabView.setScaleY(mMinScale);
//                        }
//                    }
//                }
            }
